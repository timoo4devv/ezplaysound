...
class ezplaysoundException(Exception):
    pass
...
def _ezplaysoundWin(sound, block=True):
    '''
    ...
    def winCommand(*command):
        ...
        if errorCode:
            ...
            raise ezplaysoundException(exceptionMessage)
        return buf.value
...
def _ezplaysoundOSX(sound, block=True):
    '''
    ...
    try:
        ...
        logger.warning("ezplaysound could not find a copy of AppKit - falling back to using macOS's system copy.")
    ...
    if not url:
        raise ezplaysoundException('Cannot find a sound with filename: ' + sound)
    ...
    for i in range(5):
        ...
        raise ezplaysoundException('Could not load sound with filename, although URL was good... ' + sound)
    nssound.play()
...
def _ezplaysoundNix(sound, block=True):
    ...
    if sound.startswith(('http://', 'https://')):
        ...
        path = abspath(sound)
        if not exists(path):
            raise ezplaysoundException(u'File not found: {}'.format(path))
        playbin.props.uri = 'file://' + pathname2url(path)

    set_result = playbin.set_state(Gst.State.PLAYING)
    if set_result != Gst.StateChangeReturn.ASYNC:
        raise ezplaysoundException(
...
def _ezplaysoundAnotherPython(otherPython, sound, block=True, macOS=False):
    '''
    ...
    ezplaysoundPath = abspath(getsourcefile(lambda: 0))
    t = PropogatingThread(
        target=lambda: check_call([otherPython, ezplaysoundPath, _handlePathOSX(sound) if macOS else sound]))
    t.start()
    if block:
        t.join()
...
if system == 'Windows':
    ezplaysound = _ezplaysoundWin
elif system == 'Darwin':
    ezplaysound = _ezplaysoundOSX
    import sys
    ...
    if sys.version_info[0] > 2:
        try:
            ...
            logger.warning(
                "ezplaysound is relying on a python 2 subprocess. Please use `pip3 install PyObjC` if you want ezplaysound to run more efficiently.")
            ezplaysound = lambda sound, block=True: _ezplaysoundAnotherPython(
                '/System/Library/Frameworks/Python.framework/Versions/2.7/bin/python', sound, block, macOS=True)
else:
    ezplaysound = _ezplaysoundNix
    if __name__ != '__main__':  # Ensure we don't infinitely recurse trying to get another python instance.
        try:
            ...
            logger.warning(
                "ezplaysound is relying on another python subprocess. Please use `pip install pygobject` if you want ezplaysound to run more efficiently.")
            ezplaysound = lambda sound, block=True: _ezplaysoundAnotherPython('/usr/bin/python3', sound, block, macOS=False)
